...
############ n ##############
def PostLidarData(angle_to_server, distance_to_server,after_distanceList_to_server):
    print("======================================")
    UNAME = 'Lidar'
    URL = 'http://192.168.2.12:9012/Lidar'
    with open("/home/pi/00_ThrusterCode/data/lidarData.dat","w") as LidarFile:
        LidarFile.write(str(angle_to_server)+'\t'+str(distance_to_server) + '\n' + str(after_distanceList_to_server) + '\n' + str("EOF"))
    LidarFile_req = open('/home/pi/00_ThrusterCode/data/lidarData.dat','rb')
    LidarFile = {'LidarFile':LidarFile_req}
    reqData = {'uname':UNAME, 'fileName':LidarFile}
    try:
        res = requests.post(URL, files=LidarFile, data=reqData)
        LidarFile_req.close()
...
while True:
    files = glob.glob('/home/pi/lidar/disk/Lidar_*.dat')
    recent_file = max(files, key=os.path.getctime)
    try:
        distance = []
        print("loop start")
        with open(recent_file, 'r') as h:
        #with (open(f"C:/Users/LGUser/Desktop/kaboat/Lidar/case/2808/test2/Lidar_{j}.dat", 'r') as h):
            ...
            if len(distance) == 360:
                ...
                for g in range(1, len(obstacle_sideAngle)-2):
                    ...
                    if obstacle_sideDistance[g] < nearObstacleRange  :
                        ...
                        for danger in range(0, len(CWKIM_dangerZone), 2):
                            if (CWKIM_pathAngle > CWKIM_dangerZone[danger] - 20) and (CWKIM_pathAngle < CWKIM_dangerZone[danger + 1] + 20):
                        ...
                        if g % 2 == 0:
                            '''
                            if obstacle_sideDistance[g] - obstacle_sideDistance[g - 1] < 0.2:
                                sidePath_XdistanceRange = 0
                            else:
                                sidePath_XdistanceRange = 0.7
                            '''
                ...
                for pat in range(0,len(pathAngle)):
                    ...
                    for ob in range(0, len(obstacle_sideAngle)-1, 2):
                        ...
                        # 장애물과 겹치는 점을 제거하는 과정
                        if obstacle_sideAngle[ob] - checkAngle <= pathAngle[pat] <= obstacle_sideAngle[ob + 1] + checkAngle or abs(pathAngle[pat]) > 60:
